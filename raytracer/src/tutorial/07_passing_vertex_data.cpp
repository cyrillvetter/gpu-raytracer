...
struct Vertex {
    glm::vec4 position; // Position MUST always be the first member.
    glm::vec4 normal;
};
const glm::vec4 UP{ 0.0, 1.0, 0.0, 0.0 };
...
int main() {
    ...
    shaderResources
        ...
        .expose(vertexBuffer, engine::ShaderStageFlags::ClosestHit)
        .expose(indexBuffer, engine::ShaderStageFlags::ClosestHit)
    ...
    engine::Shader shader("shaders/tutorial/07_passing_vertex_data.spv");
